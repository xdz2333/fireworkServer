...
import os
conn=sqlite3.connect('danmu.db',check_same_thread=False)
conn.execute('PRAGMA synchronous = OFF')
conn.execute('PRAGMA journal_mode = WAL')
conn.execute('PRAGMA cache_size = -1000')#1MB cache
conn.execute('PRAGMA mmap_size = 8388608')#8MB memory
...
def execute(expression):
    with lock:
        ...
        cursor.close()
        if expression.upper().startswith('INSERT') or expression.upper().startswith('UPDATE'):
            conn.commit()
    return value
